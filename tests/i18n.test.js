...
test('user-facing branding consistently uses FocusCut – Reading Overlay', () => {
  ['en', 'zh_TW', 'zh_CN'].forEach(locale => {
    ...
    assert.equal(messages.extName.message, 'FocusCut – Reading Overlay');
    assert.doesNotMatch(localeSource, /FocusOverlay/i);
  });

  [readme, aboutHtml, privacyHtml].forEach(source => {
    assert.match(source, /FocusCut/);
    assert.doesNotMatch(source, /FocusOverlay/i);
  });
  assert.match(aboutHtml, new RegExp(`FocusCut v${manifest.version.replaceAll('.', '\\.')}`));
});
